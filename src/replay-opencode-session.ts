...
type MessageRow = SourceRow & {
  session_id: string;
};

type PartRow = SourceRow & {
  message_id: string;
  session_id: string;
};
...
export type FrozenTranscript = {
  session: { id: string; data: JsonObject };
  ...
  sourceMessages: Array<{
    ...
    replayTimestampAdjustmentMs: number;
    data: JsonObject;
    parts: Array<{ id: string; timestamp: string | null; data: JsonObject }>;
  }>;
};
...
export function parseArgs(args: string[]): ReplayOptions {
  const required = (name: string) => getArg(args, name) ?? usage();
  ...
  if (apiKey && apiKeyEnv) throw new Error('--api-key and --api-key-env are mutually exclusive');
...
function parseObject(label: string, raw: string): JsonObject {
  try {
...
function timestamp(value: unknown, fallback?: number): Date {
  const millis = typeof value === 'number' ? value : fallback;
  if (!millis || !Number.isFinite(millis)) throw new Error(`Invalid or missing message timestamp: ${String(millis)}`);
  return new Date(millis);
}

function invocationState(value: unknown): 'partial-call' | 'call' | 'result' | 'output-error' {
  if (value === 'completed' || value === 'result') return 'result';
  if (value === 'error' || value === 'output-error') return 'output-error';
  if (value === 'pending' || value === 'partial-call') return 'partial-call';
  return 'call';
}

function toolPart(part: JsonObject, createdAt?: number): MastraMessagePart | null {
  const legacy = part.toolInvocation && typeof part.toolInvocation === 'object'
    ? part.toolInvocation as JsonObject
    : part;
  const stateObject = part.state && typeof part.state === 'object' ? part.state as JsonObject : undefined;
  ...
  const toolInvocation = state === 'result'
    ? { ...base, state, result: stateObject?.output ?? legacy.result }
    : state === 'output-error'
      ? { ...base, state, errorText: String(stateObject?.error ?? legacy.errorText ?? 'Tool failed') }
      : base;
  return { type: 'tool-invocation', toolInvocation, createdAt } as MastraMessagePart;
}

function convertPart(part: JsonObject, createdAt?: number): MastraMessagePart | null {
  const type = part.type;
  if (type === 'text' && typeof part.text === 'string') {
    return part.text.trim().length > 0 ? { type: 'text', text: part.text, createdAt } as MastraMessagePart : null;
  }
  if (type === 'reasoning') {
    ...
    return typeof reasoning === 'string' && reasoning.trim().length > 0
      ? { type: 'reasoning', reasoning, createdAt } as MastraMessagePart
      : null;
  }
  if (type === 'tool' || type === 'tool-invocation') return toolPart(part, createdAt);
  ...
  if (type === 'image') {
    ...
    return image !== undefined ? { type: 'image', image, createdAt } as unknown as MastraMessagePart : null;
  }
  return null;
}
...
export function loadFrozenTranscript(
  dbPath: string,
  sessionId: string,
  ...
  try {
    const sessionRow = db.query('select * from session where id = ?').get(sessionId) as JsonObject | null;
    if (!sessionRow) throw new Error(`Session not found in source database: ${sessionId}`);
    ...
    while (maxMessages === undefined || messages.length < maxMessages) {
      const rows = cursor
        ? db.query(`
            ...
            limit ?
          `).all(sessionId, cursor.time, cursor.time, cursor.id, chunkSize) as MessageRow[]
        : db.query(`
            select id, session_id, time_created, data
            from message where session_id = ?
            order by time_created asc, id asc
            limit ?
          `).all(sessionId, chunkSize) as MessageRow[];
      ...
      const partRows = db.query(`
        ...
        order by time_created asc, id asc
      `).all(sessionId, ...rows.map((row) => row.id)) as PartRow[];
      const partsByMessage = new Map<string, PartRow[]>();
      for (const row of partRows) partsByMessage.set(row.message_id, [...(partsByMessage.get(row.message_id) ?? []), row]);

      for (const row of rows) {
        const data = parseObject(`message ${row.id}`, row.data);
        const role = String(data.role ?? '');
        const originalCreatedAt = timestamp((data.time as JsonObject | undefined)?.created, row.time_created);
        ...
        const parts = rawParts
          .map((part) => convertPart(part.data, part.timestamp ? Date.parse(part.timestamp) : undefined))
          .filter((part): part is MastraMessagePart => part !== null);
    ...
    const sessionData = typeof sessionRow.data === 'string'
      ? parseObject(`session ${sessionId}`, sessionRow.data)
      : Object.fromEntries(Object.entries(sessionRow).filter(([key]) => key !== 'id'));
...
function sqliteFamily(path: string): string[] {
  return [path, `${path}-wal`, `${path}-shm`, `${path}-journal`];
}

function liveMemoryFamily(path: string): boolean {
...
export async function validateReplayPaths(options: ReplayOptions): Promise<void> {
  if (!existsSync(options.dbPath)) throw new Error(`Source database does not exist: ${options.dbPath}`);
  if (existsSync(options.memoryDbPath)) throw new Error(`Memory database must not exist: ${options.memoryDbPath}`);
  if (existsSync(options.outDir)) throw new Error(`Output directory must not exist: ${options.outDir}`);
...
export async function reserveMemoryDatabase(path: string): Promise<void> {
  let handle;
  try {
    handle = await open(path, 'wx');
  } catch (error) {
    const code = error && typeof error === 'object' && 'code' in error ? String(error.code) : '';
    if (code === 'EEXIST') throw new Error(`Memory database already exists: ${path}`);
    throw error;
  }
  await handle.close();
}
...
async function fileIntegrity(path: string): Promise<{ sha256: string; size: number }> {
  const info = await stat(path);
  return { sha256: await sha256File(path), size: info.size };
}

async function artifactIntegrity(outDir: string, artifacts: string[], memoryDbPath?: string) {
  return {
...
function sanitizedModelUrl(raw: string): string {
...
function checkpointMemoryDatabase(path: string): void {
...
function recordArtifact(record: Awaited<ReturnType<ObservationalMemory['getRecord']>>) {
  if (!record) throw new Error('Mastra returned no observational memory record');
  return {
...
async function historyArtifact(om: ObservationalMemory, threadId: string) {
  const history = await om.getHistory(threadId);
  return history.map(recordArtifact);
}

export async function runReplay(input: ReplayOptions): Promise<{ manifestPath: string; replayThreadId: string }> {
  ...
  const artifacts: string[] = ['frozen-transcript.json'];
  const manifest: JsonObject = {
    ...
    source: {
      ...
      messages: transcript.sourceMessages.map(({ id, role, timestamp, replayTimestamp, replayTimestampAdjustmentMs }) => ({
        id,
        role,
        timestamp,
        replayTimestamp,
  ...
  let debugEvents: ObservationDebugEvent[] = [];
  ...
  try {
    ...
    if (!storage) throw new Error('Failed to initialize disposable Mastra memory storage');
    const om = new ObservationalMemory({
      ...
      reflection: { observationTokens: 1_000_000_000 },
    ...
    for (let index = 0; index < options.observeCutoffs.length; index++) {
      ...
      debugEvents = [];
      let hookUsage: ObserveHookUsage | undefined;
      const result = await om.observe({
        threadId: replayThreadId,
        messages: structuredClone(transcript.messages.slice(0, cutoff)),
        hooks: { onObservationEnd: (hook) => { hookUsage = hook.usage; } },
      });
      if (!result.observed) throw new Error(`Observation cycle ${cycle} did not observe`);
      if (result.reflected) throw new Error(`Observation cycle ${cycle} unexpectedly auto-reflected`);
      ...
      if (cycle === options.reflectAfter) {
        ...
        if (!reflection.reflected) throw new Error(`Reflection after cycle ${cycle} did not reflect`);
    ...
    if (reservedMemory) {
      try {
        ...
        // Preserve the primary replay failure; partial DB/WAL files remain diagnostic evidence.
      }
    }
    ...
    manifest.artifacts = artifacts;
    manifest.integrity = await artifactIntegrity(
      options.outDir,
      artifacts,
      reservedMemory && existsSync(options.memoryDbPath) ? options.memoryDbPath : undefined,
    );
    await writeJson(manifestPath, manifest);
    throw error;
  }
